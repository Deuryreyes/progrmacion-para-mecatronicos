...
// Definición de la estructura domicilio utilizando typedef
typedef struct {
    char calle[20];
    int numero;
    int cp;
...
// Definición de la estructura empleado que contiene un campo de tipo domicilio
struct empleado {
    char nombre[20];
...
// Prototipo de función Lectura que lee los campos de un empleado
...
void main(void) {
    // Declaración e inicialización de variables de tipo estructura empleado
    struct empleado e0 = {"Arturo", "Compras", 15500.75, {"San Jerónimo", 120, 3490, "Toluca"}};
    struct empleado *e1, *e2, e3, e4;

    // Asignación de memoria dinámica para e1 y lectura de datos
    ...
    // Lectura de datos para e3 directamente en la función main
    ...
    // Asignación de memoria dinámica para e2 y e4, y llamada a la función Lectura
